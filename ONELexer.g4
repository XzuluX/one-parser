lexer grammar ONELexer;

tokens { INDENT, DEDENT }

channels { COMMENTS_CHANNEL, DIRECTIVE }

options { superClass = ONELexerBase; }
...
SINGLE_LINE_COMMENT:         '//'  ~[\r\n\f]*       -> channel(COMMENTS_CHANNEL);
SPECIAL_LINE_COMMENT:        '---' ~[\r\n\f]*       -> channel(COMMENTS_CHANNEL);
...
// function block
FUNCTIONBLOCK:  'functionblock'   { this.OnFunctionBlock(); };
INPUT:          'input'           { this.InsideFunctionBlock() }?;
OUTPUT:         'output'          { this.InsideFunctionBlock() }?;
PERSISTENT:     'persistent'      { this.InsideFunctionBlock() }?;
LOCALSETTING:   'local setting';
GLOBALSETTING:  'global setting';
VALIDRUNMODES:  'ValidRunModes'   { this.InsideFunctionBlock() }?;
DEFAULTRUNMODE: 'DefaultRunMode'  { this.InsideFunctionBlock() }?;
...
// state machine
STATEMACHINE: 'statemachine'  { this.OnStateMachine(); };
STATE:        'state'         { this.InsideStateMachine() }?;
ENTRY:        'entry'         { this.InsideStateMachine() }?;
DURING:       'during'        { this.InsideStateMachine() }?;
EXIT:         'exit'          { this.InsideStateMachine() }?;
TRIGGER:      'trigger'       { this.InsideStateMachine() }?;
SEND:         'send'          { this.InsideStateMachine() }?;
PARALLEL:     'parallel'      { this.InsideStateMachine() }?;

// counter
COUNTER:      'counter'      { this.InsideFunctionBlock() }?;

// workaround to suppress error: Cannot create implicit token for string literal in non-combined grammar
GLOBAL:       'global';
REQUIRED:     'required';
INIT:         'init';
MANAGED:      'managed';
...
CHECKSUM:     'checksum';
DUMMY:        'see lexical specification';
...
SHORT_STRING:                       '\'' (~'\'' | '\'\'')* '\'';
VERBATIM_STRING:                    '@"' (~'"'  | '""')* '"';
RAW_STRING_LITERAL_START:           '""' '"'+ { this.OnCountQuotesStart(); } -> pushMode(RAW_STRING_LITERAL);
INTERPOLATED_RAW_STRING_LITERAL_START:  '$'* { this.OnCountDollarStart(); } '""' '"'+ { this.OnCountQuotesStart(); } -> pushMode(RAW_INTERPOLATION_STRING);
INTERPOLATED_REGULAR_STRING_START:  '$"' { this.OnInterpolatedRegularStringStart(); } -> pushMode(INTERPOLATION_STRING);
INTERPOLATED_IMPLICIT_REGULAR_STRING_START: '$\'' { this.OnInterpolatedRegularStringStart(); } -> pushMode(IMPLICIT_INTERPOLATION_STRING);
INTERPOLATED_VERBATIM_STRING_START: '$@"'  { this.OnInterpolatedVerbatimStringStart(); }  -> pushMode(INTERPOLATION_STRING);

//B.1.9 Operators And Punctuators
OPEN_BRACE:               '{' { this.OnOpenBraceCheckInterpolated();  };
CLOSE_BRACE:              '}' { this.OnCloseBraceCheckInterpolated(); };
OPEN_BRACKET:             '[' { this.OnOpenBrace();  };
CLOSE_BRACKET:            ']' { this.OnCloseBrace(); };
OPEN_PARENS:              '(';
CLOSE_PARENS:             ')';
DOT:                      '.';
COMMA:                    ',';
...
OP_RIGHT_SHIFT:           '>>'  { this.IsOperator() }?;
...
mode IMPLICIT_INTERPOLATION_STRING;

IMPLICIT_DOUBLE_CURLY_INSIDE:   '{{';
IMPLICIT_OPEN_BRACE_INSIDE:     '{' { this.OpenBraceInside(); } -> skip, pushMode(DEFAULT_MODE);
IMPLICIT_REGULAR_CHAR_INSIDE:    { this.IsRegularCharInside() }? '\\';
IMPLICIT_DOUBLE_QUOTE_INSIDE:    { this.IsRegularCharInside() }? '\'\'';
IMPLICIT_SINGLE_QUOTE_INSIDE:   '\''{ this.OnDoubleQuoteInside(); } -> popMode;
IMPLICIT_REGULAR_STRING_INSIDE:  { this.IsRegularCharInside() }? ~('{' | '\\' | '\'')+;
...
mode RAW_STRING_LITERAL;

RAW_STRING_LITERAL_END:     '"'+ { this.IsRawStringLiteralEnd() }?;
RAW_STRING_LITERAL_INSIDE:  '"'+;
RAW_STRING_CONTENT:        ~'"'+;

mode RAW_INTERPOLATION_STRING;

RAW_INTERPOL_OPEN_BRACE_INSIDE:     OpenBrace   { this.OnRawStringOpenBrace(); };
RAW_INTERPOL_CLOSED_BRACE_INSIDE:   ClosedBrace;
RAW_INTERPOL_STRING_CONTENT:        ~('"' | '{' | '}')+;
RAW_INTERPOL_STRING_LITERAL_END:    '"'+ { this.IsRawStringLiteralEnd() }?;
RAW_INTERPOL_STRING_LITERAL_INSIDE: '"'+;

fragment OpenBrace:   '{';
fragment ClosedBrace: '}';
