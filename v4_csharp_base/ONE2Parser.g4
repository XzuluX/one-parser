...
parser grammar ONE2Parser;

options {
    tokenVocab = ONE2Lexer;
    superClass = ONE2ParserBase;
}

// entry point
compilation_unit
    : BYTE_ORDER_MARK? (extern_alias_directive NEWLINE)* (using_directive NEWLINE)* (global_attribute_section NEWLINE)* namespace_member_declarations? EOF
    ;
...
argument
    : (identifier ':')? refout = (REF | OUT | IN)? (expression | expression (VAR | type_))
    ;
...
simple_embedded_statement
    ...
    | SWITCH expression ':' NEWLINE INDENT (switch_section NEWLINE?)+ DEDENT # switchStatement

    // iteration statements
    | WHILE expression embedded_statement                                            # whileStatement
    | DO embedded_statement WHILE expression                                         # doStatement
    | FOR for_initializer? ';' expression? ';' for_iterator? embedded_statement      # forStatement
    | AWAIT? FOREACH identifier local_variable_type IN expression embedded_statement # foreachStatement
    ...
    | LOCK expression embedded_statement                                     # lockStatement
    | USING resource_acquisition embedded_statement                          # usingStatement
    | YIELD (RETURN expression | BREAK)                                      # yieldStatement

    // unsafe statements
    | UNSAFE block                                                           # unsafeStatement
    | FIXED fixed_pointer_declarators embedded_statement                     # fixedStatement
    ;
...
local_variable_declaration
    : (USING | REF | REF READONLY)? identifier (
        ',' identifier { this.IsLocalVariableDeclaration() }?
    )* local_variable_type multiple_local_varibale_initializer?
    | FIXED fixed_pointer_declarators
    ;
...
multiple_local_varibale_initializer
    : '=' REF? local_variable_initializer (',' local_variable_initializer)*
    ;
...
switch_section
    : switch_label+ NEWLINE INDENT (statement NEWLINE?)+ DEDENT
    ;
...
specific_catch_clause
    : CATCH class_type identifier? exception_filter? block
    ;
...
namespace_body
    : ':' NEWLINE INDENT (extern_alias_directive NEWLINE)* (using_directive NEWLINE)* namespace_member_declarations? DEDENT
    ;
...
class_base
    : OP_DERIVATION class_type (',' namespace_or_type_name)*
    ;
...
class_body
    : ':' NEWLINE INDENT (class_member_declaration NEWLINE?)+ DEDENT
    ;
...
constant_declarators
    : identifier (',' identifier)* type_ multiple_const_initializer?
    ;

multiple_const_initializer
    : '=' expression (',' expression)*
    ;

variable_declarators
    : identifier (',' identifier)* type_ multiple_variable_initializer?
    ;

multiple_variable_initializer
...
accessor_declaration
    : attrs = attributes? mods = accessor_modifier? kind = (GET | SET) block?
    ;
...
struct_body
    : ':' NEWLINE INDENT (struct_member_declaration NEWLINE?)* DEDENT
    ;

struct_member_declaration
    : attributes? all_member_modifiers? (
        common_member_declaration
        | FIXED fixed_size_buffer_declarator+ type_
    )
    ;
...
fixed_pointer_declarators
    : identifier (',' identifier)* pointer_type multiple_pointer_initializer?
    ;

multiple_pointer_initializer
...
interpolated_verbatium_string
    : INTERPOLATED_VERBATIM_STRING_START interpolated_verbatium_string_part* DOUBLE_QUOTE_INSIDE
    ;
...
delegate_definition
    : DELEGATE identifier variant_type_parameter_list? OPEN_PARENS formal_parameter_list? CLOSE_PARENS return_type? type_parameter_constraints_clauses?
    ;

event_declaration
    : EVENT (
        variable_declarators
        | member_name type_ ':' NEWLINE INDENT event_accessor_declarations DEDENT
    )
    ;
...
property_declaration // Property initializer & lambda in properties C# 6
    : member_name type_ (
        accessor_block ('=' variable_initializer)?
        | right_arrow throwable_expression
    )
    ;
...
indexer_declaration // lamdas from C# 6
    : THIS '[' formal_parameter_list ']' type_ (
        accessor_block
        | right_arrow throwable_expression
    )
    ;
...
method_or_constructor_declaration // lamdas from C# 6
    : method_member_name type_parameter_list? OPEN_PARENS formal_parameter_list? CLOSE_PARENS type_? type_parameter_constraints_clauses? constructor_initializer? (
        method_body?
        | right_arrow throwable_expression
    )
    ;
